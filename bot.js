...
// --- Links ---
const OTP_GROUP_LINK = process.env.OTP_GROUP_LINK || "[https://t.me/fast_otp0](https://t.me/fast_otp0)";     
const CHANNEL_CHAT_LINK = process.env.CHANNEL_CHAT_LINK || "[https://t.me/fastotpchat](https://t.me/fastotpchat)"; 
...
function readDb() {
    if (!fs.existsSync(DB_FILE)) {
        ...
        const defaultApis = [
            "[https://panel.lamix.org/api/v1/messages?token=F03xiioltLwvYbIlS470UQ9XuVWkp-KlqC5f_r2yXhs](https://panel.lamix.org/api/v1/messages?token=F03xiioltLwvYbIlS470UQ9XuVWkp-KlqC5f_r2yXhs)",
            "[http://169.58.213.56/crapi/ALEIUS/viewstats?token=dsglfRknOC2y_94AAD164B4ED](http://169.58.213.56/crapi/ALEIUS/viewstats?token=dsglfRknOC2y_94AAD164B4ED)"
        ];
...
// Inline Action Fix for grammY Copy Button
bot.callbackQuery(/^copy_/, async (ctx) => {
    ctx.answerCallbackQuery({ text: "Text copied to clipboard!" }).catch(()=>{});
});
...
async function deliverNumbers(ctx, service, country, showWithPrefix = true, isRefresh = false, excludedIds = new Set()) {
    ...
    selectedNumbers.forEach((numObj) => {
        ...
        // Dynamic Copy text button using standard InlineKeyboard call
                                ...
                                if (otpCode) {
                                    ...
                                    if (!dbData.sentOtps.includes(uniqueKey)) {
                                        ...
                                        const inlineKeyboardUser = new InlineKeyboard()
                                            ...
                                            .url(`👀 Number Bot`, `[https://t.me/$](https://t.me/$){botUsername}`)
    ...
    if (state.step === "input_new_service" && ctx.message.text) {
        ...
        if (ctx.message.document) {
            try {
                ...
                const response = await fetch(`[https://api.telegram.org/file/bot$](https://api.telegram.org/file/bot$){BOT_TOKEN}/${file.file_path}`);
